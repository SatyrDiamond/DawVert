...
# ------- convert -------
def convert_multiple_single(cvpj_multiple):
    print('Function-ProjConv | Converting Multiple to Single')
    cvpjm_json = json.loads(cvpj_multiple)
    if cvpjm_json['convprojtype'] == 'multiple':
        cvpjm_playlist_json = cvpjm_json['playlist']
        cvpjm_instrument_json = cvpjm_json['instruments']
        output_tracks = []
        seperate_track_from_instruments(cvpjm_playlist_json[1], cvpjm_instrument_json)
        for cvpjm_track_json in cvpjm_playlist_json:
        ...
        cvpj_json_out['convprojtype'] = 'single'
        song.sort_by_cvpjm_inst(cvpj_json_out)
        return json.dumps(cvpj_json_out)
