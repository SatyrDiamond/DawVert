...
class song_compat:
    ...
    def makecompat(self, convproj_obj, cvpj_type, in_dawinfo, out_dawinfo):
        ...
        if self.currenttime == False:
            self.process_part('autopl_addrem', autopl_addrem,         convproj_obj, cvpj_type, in_dawinfo.auto_types, out_dawinfo.auto_types)
            self.process_part('loops_remove', loops_remove,           convproj_obj, cvpj_type, in_dawinfo.placement_loop, out_dawinfo.placement_loop)
            self.process_part('sep_nest_audio', sep_nest_audio,       convproj_obj, cvpj_type, in_dawinfo.audio_nested, out_dawinfo.audio_nested)
            self.process_part('changestretch', changestretch,         convproj_obj, cvpj_type, in_dawinfo.audio_stretch, out_dawinfo.audio_stretch)
            self.process_part('removecut', removecut,                 convproj_obj, cvpj_type, in_dawinfo.placement_cut, out_dawinfo.placement_cut)
            self.process_part('loops_add', loops_add,                 convproj_obj, cvpj_type, in_dawinfo.placement_loop, out_dawinfo.placement_loop)
            self.process_part('nopl_track', nopl_track,               convproj_obj, cvpj_type, in_dawinfo.track_nopl, out_dawinfo.track_nopl)

        if cvpj_type in ['r']:
            self.process_part('time_seconds', time_seconds,               convproj_obj, cvpj_type, in_dawinfo.time_seconds, out_dawinfo.time_seconds)