...
class wavtool_project:
	def __init__(self, pd):
		...
		self.deviceRouting = {}
		self.devices = devicenodes.cvpj_devnode_semisep()
		...
		if pd != None:
			...
			if 'devices' in pd: 
				for dev_id, dev_data in pd['devices'].items():
					...
					for n, d in dev_data.items():
						if n == 'name': device_obj.visual.name = d
						elif n == 'x': device_obj.window.pos_x = d
						elif n == 'y': device_obj.window.pos_y = d
						elif n == 'type': device_obj.type = d
						elif n == 'sourceId': device_obj.subtype = d
			...
			if 'deviceRouting' in pd: 
				for dev_to, dev_from in pd['deviceRouting'].items():
					...
					self.devices.add_cable_data(from_t, from_i, to_t, to_i)
	...
	def write(self):
		...
		for trackid, trackdevices in self.devices.tracks.items():
			for dev_id, device_obj in trackdevices.items():
				...
				device_data['name'] = device_obj.visual.name
				...
				if device_obj.type in 'PortalIn':
					...
					device_data['x'] = device_obj.window.pos_x
					device_data['y'] = device_obj.window.pos_y
				...
				if device_obj.type == 'JS':
					if ordering == 1:
						device_data['sourceId'] = device_obj.subtype
						device_data['trackId'] = trackid
						device_data['x'] = device_obj.window.pos_x
						device_data['y'] = device_obj.window.pos_y
						for n, d in device_obj.data.items(): device_data[n] = d
