...
def get_param(xmldata, varname, vartype, fallback, i_loc, i_addmul): 
	if len(xmldata.findall(varname)) != 0:
		...
		if vartype == 'string': outdata = out_value
		if vartype == 'float': outdata = float(out_value)
		if vartype == 'int': outdata = int(out_value)
		if vartype == 'bool': outdata = ['false','true'].index(out_value)
