...
def get_wavestr(xmltag, x_name):
	sampleshape = xmltag.get(x_name)
	if sampleshape:
...
def lmms_decodeplugin(convproj_obj, trkX_insttr):
	...
	if len(xml_a_plugin) != 0: 
		...
		if pluginname == "sf2player":
			...
			bank = int(xml_plugin.get('bank'))
			patch = int(xml_plugin.get('patch'))
		...
		#		convproj_obj.params.add(varname[1], plugval, 'int', varname[1])
			...
			if pluginname == "papu":
				wave_data = get_wavestr(xml_plugin, 'sampleShape')
			...
			if pluginname == "watsyn":
				for shapename in ['b1_wave', 'a2_wave', 'b2_wave', 'a1_wave']:
					wave_data = get_wavestr(xml_plugin, shapename)
			...
			if pluginname == "vibedstrings":
				for num in range(9):
					...
					wave_data = get_wavestr(xml_plugin, graphid)
...
def lmms_decode_inst_track(convproj_obj, trkX, trackid):
	...
	if len(xml_a_midiport) != 0:
		...
		if midiportX.get('outputprogram') != None: track_obj.midi.out_inst.patch = int(midiportX.get('outputprogram'))
		if midiportX.get('fixedoutputvelocity') != None: track_obj.midi.out_fixedvelocity = int(midiportX.get('fixedoutputvelocity'))
		if midiportX.get('fixedoutputnote') != None: track_obj.midi.out_inst.key = int(midiportX.get('fixedoutputnote'))
...
def lmms_decode_effectslot(convproj_obj, fxslotX):
	...
	if fxpluginname == 'vsteffect':
		...
		if fxpluginname == 'delay':
			...
			delay_obj.feedback[0] = FeebackAmount
			...
			plugin_obj = delay_obj.to_cvpj(convproj_obj, None)
		else:
