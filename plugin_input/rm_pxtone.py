    ...
    def parse(self, convproj_obj, input_file, dv_config):
        ...
        while song_filesize > song_file.tell():
            ...
            if chunkname == b'MasterV5':
                ...
                wavfile_obj.set_freq(ptcop_pcm_hz)
                wavfile_obj.data_add_data(ptcop_pcm_bits, ptcop_pcm_ch, False, ptcop_pcm_data)
                wavfile_obj.write(wave_path)

                if ptcop_pcm_loop == 1: 
                    wavfile_obj.add_loop(0, ptcop_pcm_samples)
        ...
        for unit_eventnum in ptcop_unit_events:
            ...
            for unit_event in ptcop_unit_events[unit_eventnum]:
                ...
                if unit_event[2] == 5: 
                    convproj_obj.automation.add_autotick(['track', cvpj_trackid, 'vol'], 'float', unit_event[0], unit_event[3]/128)
                if unit_event[2] == 15: 
                    convproj_obj.automation.add_autotick(['track', cvpj_trackid, 'pan'], 'float', unit_event[0], ((unit_event[3]/128)-0.5)*2)
                if unit_event[2] == 14: 
                    convproj_obj.automation.add_autotick(['track', cvpj_trackid, 'pitch'], 'float', unit_event[0], math.log2(get_float(unit_event[3]))*12)
            ...
            out_vol = convproj_obj.automation.get_paramval_tick(['track', cvpj_trackid, 'vol'], firstnote, 1)
            out_pan = convproj_obj.automation.get_paramval_tick(['track', cvpj_trackid, 'pan'], firstnote, 0)
            out_pitch = convproj_obj.automation.get_paramval_tick(['track', cvpj_trackid, 'pitch'], firstnote, 0)
        ...
        for unitnum in t_notelist:
            ...
            for t_inst, t_pos, t_dur, t_key, t_vol, t_extra, t_slide in cvpj_notelist:
                track_obj.placements.notelist.add_m(t_inst, t_pos, t_dur, t_key, t_vol, t_extra)
                for tsl in t_slide: 
                    track_obj.placements.notelist.last_add_slide(tsl[0], tsl[1], tsl[2], t_vol, t_extra)
