...
class input_it(plugin_input.base):
    ...
    def parse(self, input_file):
        ...
        if header_magic != b'IMPM':
            print('Input-ImpulseTracker | Not an IT File')
            exit()
        
        header_songname = it_file.read(26).split(b'\x00' * 1)[0].decode("utf-8")
        print("Input-ImpulseTracker | Song Name: " + str(header_songname))
        ...
        for offset_pattern in offset_patterns:
            print("Input-ImpulseTracker | Pattern " + str(patterncount),end=': ')
            patterntable_single = []
            if offset_pattern != 0:
        ...
        while 255 in orders:
            orders.remove(255)
        print("Input-ImpulseTracker | Order List: " + str(orders))
        
        outputplaylist = []
        for current_channelnum in range(64):
            print('Input-ImpulseTracker | Channel ' + str(current_channelnum+1) + ': ', end='')
