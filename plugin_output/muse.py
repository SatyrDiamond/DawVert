...
class output_cvpj(plugin_output.base):
    ...
    def parse(self, convproj_json, output_file):
        ...
        cvpj_mastertrack = projJ['mastertrack']
        ...
        if 'name' in cvpj_mastertrack: addvalue(x_AudioOutput, 'name', cvpj_mastertrack['name'])
        else: addvalue(x_AudioOutput, 'name', 'Out')
        addvalue(x_AudioOutput, 'record', 0)
        if 'muted' in cvpj_mastertrack: addvalue(x_AudioOutput, 'mute', cvpj_mastertrack['muted'])
        else: addvalue(x_AudioOutput, 'mute', 0)
