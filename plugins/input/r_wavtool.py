		...
		for deviceid, devicedata in device_track.items():
			...
			if devicedata.type == 'JS':
				...
				if devicedata.sourceId == 'df142a04-31c6-4495-a8a4-c49f8429d557':
					...
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'synth-osc')
					instrument_dev = deviceid
					...
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'limiter')
					...
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'clipper')
					plugin_obj.params.add('pregain', inputdata['gain'] if 'gain' in inputdata else 0, 'float')
					effects.append(deviceid)

				elif devicedata.sourceId == 'ad949dc9-c921-429d-b08e-c62eaae5e382':
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'compressor')
					...
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'gate')
					...
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'reverb')
					...
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'convolver')
					plugin_obj.fxdata_add(1, inputdata['mix'] if 'mix' in inputdata else 1)
					...
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'bitcrush')
					...
					plugin_obj = convproj_obj.add_plugin(deviceid, 'universal', 'flanger')
